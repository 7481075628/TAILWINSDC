...
public class CheckstyleTool implements Tool {
   ...
   private class AfterExec implements PostExecAction, OutputListener {
   ...
   /** Compiled pattern for matching filenames in Checkstyle error message. **/
   private static Pattern errPattern = Pattern.compile(errPatternText);
   
   /** The location of this tool class, or null if it is not located in a file. **/
   ...
   private ToolAction sepAction = 
      new AbstractToolAction(null) {
         ...
         public boolean showMenuItem() {
            return checkFileAction.showMenuItem() || checkProjectAction.showMenuItem()
   ...
   private ToolAction checkFileAction = 
      new AbstractToolAction("Check File", "Check File", "Run Checkstyle on file", new CheckstyleFileIcon()) {
   ...
   private ToolAction checkProjectAction = 
      new AbstractToolAction("Check Project Files", "Check Project", "Run Checkstyle on project files",
         new CheckstyleFileIcon(), EnumSet.of(ToolAction.Flag.PROJECT)) {
   ...
   private ToolAction checkDirectoryAction = 
      new AbstractToolAction("Check Directory", "Check Directory",
         "Run Checkstyle on all Java files in same directory as active file", new CheckstyleDirIcon()) {
   ...
   /** True if the current conditions indicate a project containing at least one Java file, false otherwise. **/
   ...
   public CheckstyleTool(final ToolCreateData tcd) {
      creationData = tcd;
      settings = new CheckstyleSettings(tcd.getData(CheckstyleTool.class, SETTINGS_ID));
   }
    ...
    *
    *  @return the classpath necessary to run Checkstyle on the specified files. **/
   private String getRunClasspath(final List<File> files, final ActionContext context) {
      ...
      for (File f : files) {
         String dir = ToolUtil.getPackageRoot(f, context.getCharset());
         if (dir != null) {
   ...
   private void runCheckstyle(final ActionContext context, final List<File> files, final Highlightable h) {
      List<File> filesTested = new ArrayList<File>();
      ...
      context.executeCommand(command.toString(), ExecType.CHECK, "f1f1-" + errPatternText, homeFile.getAbsolutePath(),
            "PATH+=%<JAVA_BIN_DIR>%;\nPATH+=%<JGRASP_PATHS>%;", "Checkstyle", h, runAction, runAction, null,
            DateTestType.NONE, null, null);
   }
   ...
   public ToolAction[] getToolActions() {
      return new ToolAction[] { checkFileAction, checkProjectAction, checkDirectoryAction, sepAction, configureAction,
            aboutAction };
   }
   ...
   private void about(final ActionContext context) {
      ...
      if (settings.getCheckstyleHome() == null) {
         text = "jGRASP plugin for the Checkstyle development tool.\n"
               + "A Checkstyle distribution was not found. Use \"Configure\" to choose the Checkstyle home directory "
               + "if Checkstyle is installed.\n"
               + "Checkstyle and information about Checkstyle are available at "
      ...
      creationData.showMessage(context.getDialogParent(), text, "About jGRASP Checkstyle Plugin");
   }
   ...
   public void settingsChanged(final CheckstyleSettings newSettings) {
      ...
      creationData.storeData(CheckstyleTool.class, SETTINGS_ID, settings.getData());
   ...
   private void processResults(final ActionContext context, final PostExecData data, final List<File> files,
         final Set<File> failedFiles) {
      long time = System.currentTimeMillis();
      for (File f : files) {
         boolean failed = failedFiles.contains(f);
         PassFailData pfd = new PassFailData(time, failed);
         StateStorage.storeData(context, dataId, f, pfd);
      }
   }
   ...
   public ResizableFlexWidthIcon getStateIcon(final ProjectState ps, final File fl, final boolean getSpacer) {
      ...
      PassFailData pfd = (PassFailData) StateStorage.retrieveData(ps, dataId, fl);
      if (pfd == null) {
         return null;
      }
      ...
      return new CheckstyleStateIcon(pfd.failed()? StateIcon.State.FAIL : StateIcon.State.PASS);
   }
