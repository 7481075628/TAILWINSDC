...
public class JUnitOutputDisplay implements Runnable {
   ...
   private static class Renderer extends JgrTreeCellRenderer {
      ...
      private static StateIcon failIcon = 
         new StateIcon(StateIcon.State.FAIL) {
            public void paintMainIcon(final Graphics2D g, final int x, final int y, final int w, final int h) {
            }
         };
      ...
      /** {@inheritDoc} **/
      public void setupComponent(final Object userValue, final boolean leaf, final boolean expanded,
   ...
   private static class LineData {
      ...
      LineData(final int startIn, final int endIn, final String toolTipIn) {
         start = startIn;
         end = endIn;
         toolTip = toolTipIn;
      }
   ...
   private static class ClickData extends LineData {
      ...
      ClickData(final String classNameIn, final String filenameIn, final int linenumIn, final int startIn,
            final int endIn, final String toolTipIn) {
   ...
   private class NodeData {
      ...
      /** The number of tests associated with the node and its children (if any). **/
      private int count;
      
      /** The number of passed tests associated with the node and its children (if any). **/
      private int passedCount;
      
      /** The number of failed tests associated with the node and its children (if any). **/
      private int failedCount;
      
      /** The number of ignored tests associated with the node and its children (if any). **/
       ...
       *
       *  @param countIn the number of tests associated with the node and its children (if any).
       *
       *  @param level the level in the tree. **/
      NodeData(final String nameIn, final int countIn, final NodeData parentIn, final int levelIn) {
         name = nameIn;
         count = countIn;
         parent = parentIn;
         level = levelIn;
       ...
       *
       *  @param assumption true if it was an assumption failure, false if it was a test failure. **/
      public void failedTest(final String description, final boolean assumption) {
      ...
      /** Called when a test is started in the node or one of its children. **/
      ...
      /** Called when a test ends for the node or one of its children.
       *
       *  @param childTime the time for a child test, or zero if the node is the finished test. **/
      ...
      public String toString() {
         ...
         StringBuilder result = new StringBuilder((name.length() == 0)? name : (name + "  "));
         result.append(count);
   ...
   private class ProgressPanel extends JPanel {
      ...
      public void paintComponent(final Graphics g) {
         ...
         if (ignoredWidth > 0) {
            ...
            g.setColor(Drawing.getColor(Colors.BLACK));
            g.drawLine(l + successWidth + ignoredWidth, t, l + successWidth + ignoredWidth, t + fHt);
         }
         
         if (failWidth > 0) {
            g.setColor(Drawing.getColor(Colors.HL_RED));
            g.fillRect(l + 1 + successWidth + ignoredWidth, t, failWidth, fHt);
            g.setColor(Drawing.getColor(Colors.BLACK));
            g.drawLine(l + successWidth + ignoredWidth + failWidth, t, l + successWidth + ignoredWidth + failWidth,
                  t + fHt);
         }
   ...
   private static Pattern errorPattern = Pattern.compile("\\s*at (\\S+)\\.[^.]+\\(([^:]+):(\\d+)\\)");
   ...
   private Map<Integer, NodeData> idToNodeData = new HashMap<Integer, NodeData>();
   ...
   /** True if the results dialog should not be popped up, false if it should be. **/
   ...
   /** True if the previous process ended before the reading of JUnit communications was finished. **/
    ...
    *
    *  @param ss the server socket for communicating with the JUnit runner.
    *
    *  @param locDataIn location data for opening files.
    *
    *  @param testRootDirsIn source root directories for test files.
    *
    *  @param dontShowDialogIn true if the results dialog should not be popped up, false if it should.
    *
    *  @param titleIn a useful title for the thing being tested.
    *
    *  @param tcd tool creation data for JUnit plugin.
    *
    *  @return false if a previous session is still running after a short wait, true otherwise. **/
   public boolean init(final ServerSocket ss, final LocData locDataIn, final Set<File> testRootDirsIn,
         final boolean dontShowDialogIn, final String titleIn, final ToolCreateData tcd) {
      /** In case the previous run has not terminated. **/
      if (running && endFlag) {
         /** Previous process has ended, but we may be stuck waiting for a connection or reading. **/
   ...
   public void run() {
      ...
      try {
         ...
         if (!endFlag) {
            showError(tcd, "Unable to set timeout on communication socket: " + ToolUtil.getDetailMessage(e));
         }
         close(serverSocket);
         synchronized (this) {
            running = false;
      ...
      try {
         ...
         showError(tcd, "Unable connect to JUnit runner after " + connectTimeout + " seconds.");
         close(serverSocket);
         return;
      }
      catch (IOException e) {
         if (!endFlag) {
            showError(tcd, "Unable to open communication socket: " + ToolUtil.getDetailMessage(e));
         }
         close(serverSocket);
         synchronized (this) {
            running = false;
      ...
      try {
         ...
         if (!endFlag) {
            showError(tcd, "Unable to open communication socket input stream: " + ToolUtil.getDetailMessage(e));
         }
         close(serviceSocket);
         close(serverSocket);
         synchronized (this) {
   ...
   private void showError(final ToolCreateData tcd, final String msg) {
      SwingUtilities.invokeLater(()->tcd.showError(PluginUtil.getDialogParent(), msg, "JUnit Plugin Error"));
   }
    ...
    *
    *  @param assumption true if this is an assumption failure, false if it is a test failure. **/
   private void handleFail(final String description, final boolean assumption) {
      ...
      if (frame == null) {
         GIconData iconData = 
            new GIconData() {
               ...
               public Paint getOutlinedBorderPaint(final int x, final int y, final int size) {
                  return null;
               }
            
               public Paint getBorderPaint(final int x, final int y, final int size) {
                  return null;
               }
            
               public Paint getBackgroundPaint(final int x, final int y, final int size) {
                  float[] fractions = new float[] { 0, .5f, 1f };
                  Color[] colors = new Color[] { Drawing.getColor(Colors.HL_RED), Drawing.getColor(Colors.WHITE),
                        Drawing.getColor(Colors.HL_GREEN) };
                  return new LinearGradientPaint(x + size / 5, y + size / 5, x + size - size / 5, y + size - size / 5,
                        fractions, colors);
               }
         ...
         frame = new JGraspFrame("JUnit Results", iconData, PluginUtil.getDialogParent().getGraphicsConfiguration());
         frame.setAat(true);
         ...
         tree.addTreeSelectionListener(
            (e)-> {
               ...
               setSelection((tp == null)? null : (TreeNode) tp.getLastPathComponent());
            });
         ...
         JSplitPane splitPane = 
            new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, new JScrollPane(tree), rightPane) {
               public Dimension getPreferredSize() {
   ...
   private void handleTextMouseClick(final MouseEvent e) {
      ...
      int pos = tp.viewToModel2D(new Point2D.Double(e.getX(), e.getY()));
      Element el = tp.getStyledDocument().getCharacterElement(pos);
      ClickData cd = (ClickData) el.getAttributes().getAttribute(LINK_ATTRIBUTE);
      if (cd == null) {
         return;
      }
      tp.setSelectionStart(((LineData) cd).start);
      tp.setSelectionEnd(((LineData) cd).end);
      PluginUtil.openClass(cd.className, cd.filename, cd.linenum, locData, testRootDirs);
   }
   ...
   private void handleTextMouseMoved(final MouseEvent e) {
      ...
      Element el = tp.getStyledDocument().getCharacterElement(
            tp.viewToModel2D(new Point2D.Double(e.getX(), e.getY())));
      ClickData cd = (ClickData) el.getAttributes().getAttribute(LINK_ATTRIBUTE);
      LineData ld = cd;
      if (ld == null) {
      ...
      if (ld != null) {
         try {
            Rectangle2D endChar = tp.modelToView2D(ld.end);
            int endX = (int) Math.ceil(endChar.getX() + endChar.getWidth());
    ...
    *
    *  @param node the new selected node, or null if there is no selection. **/
   private void setSelection(final TreeNode node) {
      ...
      NodeData nd = (NodeData) ((DefaultMutableTreeNode) node).getUserObject();
      ...
      for (int l = 0; l < lines.length; l++) {
         ...
         try {
            ...
            if (line.startsWith(headerMarker)) {
               ...
               boldStyle.addAttribute(LINE_ATTRIBUTE, new LineData(doc.getLength(), doc.getLength() + line.length(),
                     line.trim()));
               style = boldStyle;
            }
            else {
               ...
               if (m.matches()) {
                  ...
                  if (mr.groupCount() == 3) {
                     ...
                     try {
                        ...
                        ClickData cd = new ClickData(mr.group(1), mr.group(2), linenum, doc.getLength(),
                              doc.getLength() + line.length(), trimmedLine);
                        style = textPane.addStyle("clickable", null);
                        StyleConstants.setForeground(style, Drawing.createColor(linkColor));
   ...
   /** Recursively parses a test description node, creating the tree and populating the id-to-node data mapping.
    *
    ...
    *  string.
    *
    *  @param parent parent node, or null if the description for the root node will be parsed.
    *
    *  @param parentData parent node data, or null if the description for the root node will be parsed.
    *
    *  @param idToData map from nodes id to node data.
    *
    *  @param level the level in the tree.
    *
    *  @return the position at the end of the node description in the string.
    *
    *  @throws FormatException if the description is has an invalid format. **/
   private int parseTree(final String description, final int posIn, final DefaultMutableTreeNode parent,
         final NodeData parentData, final Map<Integer, NodeData> idToData, final int level) throws FormatException {
      int pos = posIn;
   ...
   /** Called to indicate that the running jUnit process has ended, so any new process should be allowed to start. **/
   public void flagEnd() {
      synchronized (this) {
         if (!running) {
            return;
   ...
   /** Sets or releases a temporary always on top override state for the window.
    *
    *  @param state true if the temporary always-on-top state will be set, false if it will be released. **/
