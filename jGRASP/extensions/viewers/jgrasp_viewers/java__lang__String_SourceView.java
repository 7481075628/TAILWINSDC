...
/** A viewer that displays a java.lang.String as it could appear in source code. **/
public class java__lang__String_SourceView extends TextAreaView {
   ...
   public String getDisplayText(final Value value, final DebugContext context) throws ViewerException {
      ...
      for (int start = 0; start < result.length();) {
         ...
         if (end >= result.length()) {
            ...
            if (lastBackslash >= end - 1 || lastBackslash >= end - 6 && result.charAt(lastBackslash + 1) == 'u') {
   ...
   public void getInfo(final ViewerInfo vi) {
      ...
      vi.setLongDescription("This viewer displays a String as it would appear in Java source code. Note that if the "
            + "String does appear in source code, the format may not be exactly the same, but it will produce an "
            + "equivalent String.");
   }
