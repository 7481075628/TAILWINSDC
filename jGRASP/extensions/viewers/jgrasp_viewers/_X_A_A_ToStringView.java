...
public class _X_A_A_ToStringView extends StringTableViewWSV {
   ...
   public void getInfo(final ViewerInfo vi) {
      ...
      vi.setLongDescription("This viewer displays the toString() value for array elements in a 2D grid of cells. "
            + "Selecting a cell will cause its value to be displayed in a subviewer.");
   }
   ...
   public void update(final ViewerValueData valueData, final ViewerUpdateData data, final DebugContext context,
      ...
      List<Value> rowValues = value.getArrayElements(context, rowOffset, Math.min(numRowsShown, rows - rowOffset));
      for (int i = 0; i < numRowsShown; i++) {
         if (i + rowOffset < rows) {
            Value row = rowValues.get(i);
            int cols = (row == null || row.isNull())? 0 : row.getArrayLength(context);
            if (cols > longestCol) {
               longestCol = cols;
            }
            List<Value> colValues = (row == null || row.isNull() || cols - colOffset <= 0) ? null
                  : row.getArrayElements(context, colOffset, Math.min(numColsShown, cols - colOffset));
   ...
   private String getExpr(final ViewerValueData valueData, final int row, final int col, final DebugContext context) {
      ...
      return "((" + (cast.indexOf('.') >= 0 ? "`cast`" : "") + cast + ") " + expr + ")[" + row + "][" + col + "]";
   }
}
