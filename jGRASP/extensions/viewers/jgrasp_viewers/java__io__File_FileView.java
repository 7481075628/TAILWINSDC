...
public class java__io__File_FileView extends TextAreaView {
   ...
   public String getDisplayText(final Value value, final DebugContext context) throws ViewerException {
      ...
      Method apMethod = value.getMethod(context, "getAbsolutePath", "java.lang.String", null);
      Value ap = value.invokeMethod(context, apMethod, null);
   
      Method isAbsMethod = value.getMethod(context, "isAbsolute", "boolean", null);
      Value isAbsV = value.invokeMethod(context, isAbsMethod, null);
      boolean isAbs = isAbsV.toBoolean(context);
   
      Method existsMethod = value.getMethod(context, "exists", "boolean", null);
      Value existsV = value.invokeMethod(context, existsMethod, null);
      boolean exists = existsV.toBoolean(context);
      if (!exists) {
         return (isAbs ? "Absolute" : "Relative") + " File Path: " + ap.toString(context) + "\nExists: false";
      }
      Method isDirMethod = value.getMethod(context, "isDirectory", "boolean", null);
      Value isDirV = value.invokeMethod(context, isDirMethod, null);
      boolean isDir = isDirV.toBoolean(context);
   
      if (isDir) {
         return (isAbs ? "Absolute" : "Relative") + " Directory Path: " + ap.toString(context);
      }
