...
public class java__awt__Component_ComponentView extends ViewerRoot {
   ...
   private static class Bounds {
      ...
      private void getDescription(final StringBuilder sb, final String indent) {
         sb.append(indent);
         sb.append(type);
         sb.append(" (");
         sb.append(bounds.x);
   ...
   /** The boundary hierarchy. This will be accessed by both the debugger and gui threads, so it should be read only
    * once wherever the value must remain consistent. **/
   ...
   /** The id of the currently selected component, or -1 if none is selected. **/
   ...
   /** Creation data. **/
   private final ViewerCreateData createData;
    ...
    *
    * @param old if this is non-negative and a component with this id is found, select the component found after it,
    * if any. Using this repeatedly will cycle through all the components at a certain position.
    *
    * @param keep if this is non-negative and <code>old</code> is negative, choose the component with this id if it is
    * one of the ones found.
    *
    * @return the component found at the specified position, or null if none was found. **/
   public Value findComponent(final Point pos, final long old, final long keep) {
      ...
      double scale = Math.min(width / (double) b.width, height / (double) b.height);
      ...
      for (int i = 0; i < result.size(); i++) {
         if (result.get(i).id == old) {
            return i > 0 ? result.get(i - 1).value : result.get(result.size() - 1).value;
         }
      }
      return result.get(result.size() - 1).value;
   }
   ...
   public void getInfo(final ViewerInfo vi) {
      ...
      vi.setLongDescription("This viewer displays the component shape and the shapes of its descendants. Clicking on "
            + "the viewer repeatedly will cycle the selection through all components under the cursor. Hold down the "
            + "shift key to click and drag out a component without cycling to the next component.");
   }
   ...
   /** Changes the value in the selection viewer based on the current selected value.
    *
    * @param context the current debug context.
    *
    * @param isReset true if the current value should be considered a "new" value, false otherwise. **/
   public void setSelectionViewValue(final DebugContext context, final boolean isReset) {
      ...
      SwingUtilities.invokeLater(
         ()-> {
            if ((v == null) != prevNullSubviewVal) {
               getVIData().setSubviewPanel(v == null ? null : selectionViewer.getComponent(),
   ...
   private void buildPanel() {
      ...
      displayPanel.addMouseListener(
         new MouseAdapter() {
            ...
            public void mouseExited(final MouseEvent e) {
               mouseOut = true;
               Bounds b = topBounds;
               setTextAreaText(b == null ? "" : b.toString());
            }
         
         
            @Override
            public void mousePressed(final MouseEvent e) {
               if ((e.getModifiersEx() & BasicToolUtil.getMenuShortcutKeyMaskEx()) != 0) {
                  newSelComponent = null;
               }
               else {
                  long id = (e.getModifiersEx() & InputEvent.SHIFT_DOWN_MASK) == 0 ? selId : -1;
                  long keep = (e.getModifiersEx() & InputEvent.SHIFT_DOWN_MASK) == 0 ? -1 : selId;
      ...
      ValueDragger.createValueDragger(displayPanel, getVIData(), 
         (pos)-> {
            ...
            return new ValueDragData(null, selComponent, null, getVIData().getScope(),
                    getClass().getName());
         });
   ...
   private static void findComponents(final int x, final int y, final double xOffset, final double yOffset,
         final double scale, final Bounds bounds, final int level, final List<? super Bounds> out) {
      Rectangle b = bounds.bounds;
      ...
      Rectangle rect = new Rectangle((int) xOffs, (int) yOffs, (int) (xOffs + b.width * scale) - (int) xOffs,
   ...
   private static void findComponents(final int x, final int y, final double xOffset, final double yOffset,
         final double scale, final Bounds bounds, final int level, final StringBuilder textOut) {
      Rectangle b = bounds.bounds;
      ...
      textOut.append(" ".repeat(Math.max(0, level)));
      ...
      textOut.append("\n");
      for (Bounds element : bounds.children) {
         findComponents(x, y, xOffs, yOffs, scale, element, level + 1, textOut);
      }
   }
    ...
    *
    * @throws ViewerException if an error occurs while retrieving the bounds. **/
   private static Bounds getBounds(final Value value, final DebugContext context) throws ViewerException {
      Bounds b = new Bounds();
      Method getBoundsMethod = value.getMethod(context, "getBounds", "java.awt.Rectangle", null);
      ...
      try {
         Method getTextMethod = value.getMethod(context, "getText", "java.lang.String", null);
      ...
      if (value.isInstanceOf(context, "java.awt.Container")) {
         Method getComponentsMethod = value.getMethod(context, "getComponents", "java.awt.Component[]", null);
         Value children;
         try {
         ...
         List<Value> childValues = children.getArrayElements(context);
         for (int c = 0; c < b.children.length; c++) {
            b.children[c] = getBounds(childValues.get(c), context);
         }
      }
      return b;
   }
   ...
   private void paintBounds(final Graphics g, final double xOffset, final double yOffset, final double scale,
         final Bounds bounds, final int level, final int l) {
      Rectangle b = bounds.bounds;
      ...
      g.fillRect((int) xOffs, (int) yOffs, (int) (xOffs + b.width * scale) - (int) xOffs,
            (int) (yOffs + b.height * scale) - (int) yOffs);
      if ((int) (xOffs + b.width * scale) - (int) xOffs > l + l
            && (int) (yOffs + b.height * scale) - (int) yOffs > l + l) {
         double shade = Math.max(0, 1 - 0.05 * level);
         int rMin = 100;
         int rMax = 200;
         int i = (int) (rMin + shade * (rMax - rMin));
         Color c = bounds.id == selId ? new Color(255, i, i) : new Color(i, i, 255);
         g.setColor(Drawing.convertForTheme(c));
         g.fillRect((int) xOffs + l, (int) yOffs + l, (int) (xOffs + b.width * scale) - (int) xOffs - l - l,
               (int) (yOffs + b.height * scale) - (int) yOffs - l - l);
         for (Bounds element : bounds.children) {
