...
public class _X_A_A_ToStringView extends StringTableViewWSV {
   ...
   public void getInfo(final ViewerInfo vi) {
      ...
      vi.setLongDescription("This viewer displays element values for arrays in a 2D grid of cells. Selecting a cell "
   ...
   public void update(final ViewerValueData valueData, final ViewerUpdateData data, final DebugContext context,
      ...
      for (int i = 0; i < numRowsShown; i++) {
         if (i + rowOffset < rows) {
            ...
            int cols = row.isNull()? 0 : row.getArrayLength(context);
            if (!row.isNull()) {
